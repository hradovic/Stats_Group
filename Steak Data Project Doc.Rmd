...
tidy(glm_regincome)
```
The predicted odds that a person from the east north central region with income under \$25000 prefers rare steak is $e^{-0.55990752}=0.5712619$, or about a 36% probability.
...
steak_survey_ageincome_NArm <- completeFun(steak_survey, c("age", "hhold_income"))
anova(glm(steak_prep_rare ~ 1, data = steak_survey_ageincome_NArm, family = binomial(link = "logit")), glm_ageincome) %>%
  tidy()
```
